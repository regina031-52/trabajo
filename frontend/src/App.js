...
import { MapPin, Path, Timer, Ruler, Buildings, NavigationArrow, Warning, List, MagnifyingGlass, X } from "@phosphor-icons/react";
    ...
    <div ref={wrapperRef} className="relative" data-testid="search-wrapper">
      <div className="relative">
        ...
        <input
          ...
          placeholder="Escribe tu calle... (ej: Calle Zapata)"
      ...
      {isOpen && results.length > 0 && (
        <div
          className="absolute top-full left-0 right-0 mt-1 max-h-60 overflow-y-auto rounded-lg z-50"
          style={{
          ...
          {results.map((nodo, idx) => (
            ...
            >
              ...
              <p className="text-zinc-500 text-xs mt-0.5">
                {nodo.tipo === "escuela" ? "Refugio" : "Interseccion"} &middot; {nodo.lat.toFixed(4)}, {nodo.lon.toFixed(4)}
              </p>
            </button>
          ))}
        </div>
      ...
      {isOpen && results.length === 0 && query.length >= 2 && !loading && (
        <div
          className="absolute top-full left-0 right-0 mt-1 p-4 rounded-lg text-center"
          style={{
            background: "rgba(9, 9, 11, 0.95)",
            border: "1px solid rgba(255,255,255,0.15)"
          }}
        >
          <p className="text-zinc-400 text-sm">No se encontraron resultados</p>
        </div>
      )}
    </div>
  );
...
function App() {
  ...
  useEffect(() => {
    const cargarDatos = async () => {
      try {
        const [escuelasRes, aristasRes, statsRes] = await Promise.all([
          axios.get(`${API}/escuelas`),
          axios.get(`${API}/aristas`),
          axios.get(`${API}/grafo/stats`)
        ]);
        setEscuelas(escuelasRes.data);
        setAristas(aristasRes.data);
  ...
  // Seleccionar nodo desde búsqueda
  ...
  const calcularRuta = useCallback(async () => {
    if (!nodoSeleccionado) {
      setError("Escribe y selecciona tu ubicación primero");
      return;
    }
    ...
    try {
      const response = await axios.post(`${API}/calcular-ruta`, {
        nodo_origen: nodoSeleccionado.id
      });
  ...
  return (
    <div className="relative w-screen h-screen overflow-hidden" data-testid="app-container">
      ...
      >
        ...
        {/* Marcadores de escuelas con nombres visibles */}
            ...
            >
              ...
              >
                <span className="school-tooltip-text">
                  {escuela.alias && escuela.alias.split(";")[0].trim().length < escuela.nombre.length
                    ? escuela.alias.split(";")[0].trim()
                    : (escuela.nombre.length > 30
                      ? escuela.nombre.substring(0, 28) + "..."
                      : escuela.nombre)
                  }
                </span>
              </Tooltip>
              <Popup className="custom-popup">
                <div className="text-sm" style={{ maxWidth: 220 }}>
                  ...
                  {escuela.alias && (
                    <p style={{ color: "#A1A1AA", fontSize: 11, marginTop: 4 }}>{escuela.alias}</p>
                  )}
                  <p style={{ color: "#FF0055", fontSize: 11, marginTop: 4, textTransform: "uppercase" }}>
                    {escuela.tipo}
                    {escuela.unificada && " (doble turno)"}
                  </p>
                </div>
              </Popup>
            </CircleMarker>
          ...
          >
            ...
            <Popup>
              <div className="text-sm">
                <strong style={{ color: "#00E5FF" }}>Tu ubicacion</strong>
                <br />
                <span style={{ color: "#A1A1AA" }}>{nodoSeleccionado.nombre}</span>
              </div>
            </Popup>
          </CircleMarker>
        )}
      ...
      <div 
        className="glass-panel absolute top-6 left-6 w-96 max-h-[calc(100vh-48px)] overflow-hidden animate-slide-in z-10 flex flex-col"
      ...
      >
        <div className="p-6 space-y-5 overflow-y-auto flex-1">
          ...
          {resultado?.mejor_ruta && (
            <div className="space-y-4 animate-slide-in" data-testid="route-result">
              {/* Escuela más cercana */}
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <span className="inline-block mt-2 px-2 py-1 bg-white/10 rounded text-xs text-zinc-300 uppercase">
                      ...
                      {resultado.mejor_ruta.escuela.unificada && " - doble turno"}
                    </span>
                  </div>
                </div>
              </div>
              ...
              {/* Toggle para ver todas las rutas */}
              <button
                        ...
                        <div className="flex items-start justify-between">
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-zinc-500 text-xs">
                              {ruta.escuela.tipo}
                              {ruta.escuela.unificada && " (doble turno)"}
                            </p>
                          </div>
                          <div className="text-right flex-shrink-0 ml-2">
                            <p className="text-[#CCFF00] font-mono text-sm">
                              {ruta.distancia_total >= 1000
                                ? `${(ruta.distancia_total / 1000).toFixed(1)}km`
                                : `${Math.round(ruta.distancia_total)}m`}
                            </p>
          ...
          {!resultado && !error && (
            <div className="text-center py-4">
              <div className="w-16 h-16 mx-auto mb-4 rounded-full bg-white/5 flex items-center justify-center">
                <MagnifyingGlass weight="duotone" className="text-3xl text-zinc-500" />
              </div>
              <p className="text-zinc-500 text-sm">
                Escribe el nombre de tu calle para encontrar el refugio mas cercano
              </p>
            </div>
          )}
        </div>
